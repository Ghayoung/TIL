...
public class PersistTokenRepository implements PersistentTokenRepository {
	...
	public void createNewToken(PersistentRememberMeToken token) {
		RememberToken rememberToken = new RememberToken(token.getUsername(), token.getSeries(), token.getTokenValue(),
				token.getDate());
	...
	public PersistentRememberMeToken getTokenForSeries(String seriesId) {
		...
		try {
			RememberToken rememberToken = JsonUtils.fromJson(payload, RememberToken.class);
			PersistentRememberMeToken token = new PersistentRememberMeToken(rememberToken.getUsername(), seriesId,
					rememberToken.getTokenValue(), rememberToken.getDate());
			return token;
		} catch (Exception e) {
			logger.error("Persistent token is not valid. payload={}, error={}", payload, e);
			return null;
